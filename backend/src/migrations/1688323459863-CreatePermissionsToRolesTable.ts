...
export class CreatePermissionsToRolesTable1688323459863 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`permissions_roles\` (\`id\` int NOT NULL AUTO_INCREMENT, \`enabled\` tinyint(1) NOT NULL DEFAULT 0, \`permissionId\` int NULL, \`roleId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`permissions\` ADD UNIQUE INDEX \`IDX_379d6448e4c61af062a56ae890\` (\`signature\`)`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`permissions\` DROP INDEX \`IDX_379d6448e4c61af062a56ae890\``);
        await queryRunner.query(`DROP TABLE \`permissions_roles\``);
        await queryRunner.query(`CREATE UNIQUE INDEX \`IDX_f3fd0fd7450378041bf4395b3c\` ON \`permissions\` (\`name\`)`);
        await queryRunner.query(`CREATE UNIQUE INDEX \`IDX_e89ffea3a5fd8f54d8990e7de8\` ON \`permissions\` (\`signature\`)`);
    }
